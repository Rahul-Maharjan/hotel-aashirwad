...
const SingleRoom = () => {
    ...
    // Prepare gallery images (featured + gallery items)
    const allImages = [room.featured_image, ...(room.gallery || [])].filter(Boolean);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-[#e5e5e5]">
                    ...
                    <nav className="flex items-center gap-2 text-sm text-[#4f4f4f] uppercase tracking-wider">
                        ...
                        <span>/</span>
                        <span className="text-[#0f1f47] font-semibold">{room.room_name}</span>
                    </nav>
                        ...
                        <section className="bg-[#f7f5f1] p-6 sm:p-8 flex flex-wrap items-center justify-between gap-6 border-l-4 border-[#0f1f47]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-serif text-[#0f1f47] text-lg">{room.room_size} Sq. Ft</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-white flex items-center justify-center shadow-sm">
                                ...
                                <div>
                                    ...
                                    <p className="font-serif text-[#0f1f47] text-lg">{room.occupancy} Guests</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-white flex items-center justify-center shadow-sm">
                                ...
                                <div>
                                    ...
                                    <p className="font-serif text-[#0f1f47] text-lg">
                                        {room.beds?.map(b => `${b.quantity} ${b.type}`).join(', ') || 'Standard Bed'}
                                    </p>
                                </div>
                            </div>
                        </section>
                        ...
                        {room.special_features && room.special_features.length > 0 && (
                            <section>
                                ...
                                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                                    {room.special_features.map((feature, i) => (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                                {room.amenities?.map((amenity, index) => (
                                    <div key={index} className="flex items-center flex-col text-center p-4 border border-[#e5e5e5] bg-white hover:border-[#9b7b45] transition-colors group">
                                        ...
                                        <span className="text-[#2d2d2d] font-medium text-sm">{amenity.name}</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                            ...
                            <section className="border-t border-[#e5e5e5] pt-10">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-12">
                                        ...
                                        <div className="text-[#4f4f4f]">
                                            <p>{room.policy.cancellation_policy}</p>
                                        </div>
                                    </div>
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-12">
                                        <span className="font-semibold text-[#0f1f47] uppercase tracking-wider text-sm min-w-[150px]">House Rules</span>
                                        <div className="text-[#4f4f4f] whitespace-pre-line">
                                            {room.policy.house_rules}
                                        </div>
                                    </div>
                                </div>
                            </section>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-28 bg-[#f7f5f1] border border-[#e5e5e5] p-6 shadow-xl">
                            <div className="mb-6 pb-6 border-b border-[#d8d1c2]">
                                <h3 className="font-serif text-[#0f1f47] text-2xl uppercase tracking-wide mb-2">{room.room_name}</h3>
                                <div className="flex flex-wrap items-baseline gap-2">
                                    <span className="text-3xl font-bold text-[#9b7b45]">{room.currency} {room.price}</span>
                            ...
                            >
                                ...
                                <div className="p-6">
                                    ...
                                    <p className="text-[#6a6a6a] text-sm uppercase tracking-wider flex items-center gap-4">
                                        <span><Users className="w-4 h-4 inline mr-1 text-[#9b7b45]" /> {r.room_type}</span>
                                    </p>
                                </div>
                            </Link>
                        ))}
